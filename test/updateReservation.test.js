...
describe("Các testcase cho chức năng cập nhật đơn đặt chỗ", function () {
  ...
  // TC 16: Chỉ cập nhật nếu đơn đang 'Pending'
  it("TC 16: Chỉ cập nhật nếu đơn đang 'Pending'", async function () {
    ...
    await navToUpdateBookingPage(approvedBooking.ReservationID)
    const approvedOverlay = await driver.wait(
      until.elementLocated(By.css(".status-overlay.approved")),
      5000
    )
    expect(await approvedOverlay.getText()).to.equal("Đã duyệt đơn")

    await axios.delete(
      `${DOMAIN}/delete-sample-bookings?ReservationID=${approvedBooking.ReservationID}`
    )
    const { data: rejectedData } = await axios.get(
      `${DOMAIN}/create-sample-bookings?Status=Rejected`
    )
    const rejectedBooking = rejectedData.booking

    await navToUpdateBookingPage(rejectedBooking.ReservationID)
    const rejectedOverlay = await driver.wait(
      until.elementLocated(By.css(".status-overlay.rejected")),
      5000
    )
    expect(await rejectedOverlay.getText()).to.equal("Đã từ chối đơn")

    await axios.delete(
      `${DOMAIN}/delete-sample-bookings?ReservationID=${rejectedBooking.ReservationID}`
    )
    const { data: pendingData } = await axios.get(`${DOMAIN}/create-sample-bookings?Status=Pending`)
    const pendingBooking = pendingData.booking

    await navToUpdateBookingPage(pendingBooking.ReservationID)

    const pendingDayTime = dayjs().hour(12).minute(30)
    const pendingDate = pendingDayTime.format("DD/MM/YYYY")
    const pendingTime = pendingDayTime.format("HH:mm")

    await placeOrderOnStaticForm(driver, dataOnTestingFlow["PLACE_ORDER"](pendingDate, pendingTime))
    await waitForLooking()
  ...
  // TC 17: Luồng chuẩn, thành công (backbone)
  it("TC 17: Luồng chuẩn, thành công", async function () {
    const reservation = await seedTestReservationData("GENERAL")
    const { ReservationID } = reservation
